                    ...
                    <div class="card full-height">
                        <div class="card-header">
                            <div class="card-head-row">
                                ...
                                <a href="createkrs.php" class="btn btn-primary btn-sm ml-auto">Input KRS</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        $query_jadwal = "SELECT mk.nama_mk, jk.hari, CONCAT(jk.jam_mulai, ' - ', jk.jam_selesai) AS jam, jk.kode_kelas, rk.nama_ruangan, d.nama_dosen
																				FROM krs_mahasiswa km
																				INNER JOIN jadwal_kuliah jk ON km.kode_mk = jk.kode_mk
																				INNER JOIN matakuliah mk ON jk.kode_mk = mk.kode_mk
																				INNER JOIN ruang_kelas rk ON jk.id_ruang = rk.id_ruang
																				INNER JOIN dosen d ON jk.npk = d.npk
																				WHERE km.NIM= '$nim'";
                                        $result_jadwal = mysqli_query($kon, $query_jadwal);
																				
                                        if (mysqli_num_rows($result_jadwal) > 0) {
                                            while ($row_jadwal = mysqli_fetch_assoc($result_jadwal)) {
                                                echo "<tr>";
                                                echo "<td>" . $row_jadwal['nama_mk'] . "</td>";
                                                echo "<td>" . $row_jadwal['hari'] . "</td>";
                                                echo "<td>" . $row_jadwal['jam'] . "</td>";
                                                echo "<td>" . $row_jadwal['kode_kelas'] . "</td>";
                                                echo "<td>" . $row_jadwal['nama_ruangan'] . "</td>";
                                                echo "<td>" . $row_jadwal['nama_dosen'] . "</td>";
                                                echo "</tr>";
                                            }
                                        } else {
                                            echo "<tr><td colspan='6'>Tidak ada jadwal perkuliahan.</td></tr>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
